...
while date < df['Date'].max():
    try:
        dfb = pd.read_csv(src + 'Investor 2/Results/balances.csv')
    ...
    dfl2 = update_ledger(df3, path = src + 'Investor 2/Results/ledger.csv')
    balance = make_a_move(df3,dfl2,num_investments = num_investments, path = src + 'Investor 2/Results/ledger.csv')
    ...
    dfl_csv.to_csv(src + 'Investor 2/Results/Historical Ledger.csv')
    
    df4.to_csv(src + 'Investor 2/Results/balances.csv')
